...
void Tree::Insert(Node* newNode)
{
    ...
    else if (strcmp(newNode->name, temp->name) < 0)
    {
        temp->m_left = newNode;
    }
    else
...
void Tree::Print(const Node* node) const
{
    if (node != nullptr)
    {
        ...
        cout << node->name << '\t' << node->lastName << '\t' << node->surname<<"\t"<< node->grade << endl;
...
void Tree::Remove(Node* removeNode)
{
    if (removeNode != nullptr)
    {
        ...
        if (temp != removeNode)
        {
            ...
            strcpy_s(removeNode->name, 15U, temp->name);
            strcpy_s(removeNode->lastName, 15U, temp->lastName);
            strcpy_s(removeNode->name, 15U, temp->surname);
            strcpy_s(removeNode->lastName, 3U, temp->grade);
        }
